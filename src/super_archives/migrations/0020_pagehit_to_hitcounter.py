...
from south.logger import _logger
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Migrate all data from the PageHit model to the given object with
        the HitCounterMixin."""

        content_type = orm['contenttypes.ContentType'].objects.get(
            model='thread',
            app_label='super_archives'
        )

        for obj in orm.PageHit.objects.all():
            view = resolve(obj.url_path)

            try:
                thread = orm.Thread.objects.get(
                    subject_token=view.kwargs.get('thread_token'),
                    mailinglist__name=view.kwargs.get('mailinglist'),
                )
            except orm.Thread.DoesNotExist:
                continue

            hit_obj, created = orm['hitcounter.Hit'].objects.get_or_create(
                content_type=content_type,
                object_pk=thread.pk,
                defaults={'hits': obj.hit_count},
            )

            if not created:
                hit_obj.hits = obj.hit_count
                hit_obj.save()

    def backwards(self, orm):
        _logger.warning(u"This backwards don't get the data of PageHit back.")
    ...
    models = {
        ...
        u'super_archives.emailaddressvalidation': {
            ...
            'validation_key': ('django.db.models.fields.CharField', [], {'default': "'60d2bcbd9d924d9f8139c2d1227223ed'", 'max_length': '32', 'null': 'True'})
        },
    ...
    complete_apps = ['hitcounter', 'super_archives']
    symmetrical = True
