...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        page_hits = orm.PageHit.objects.all()
        for page_hit in page_hits:
            if page_hit.url_path.startswith('/archives/thread/'):
                token = page_hit.url_path.split('/')[-1]
                threads = orm.Thread.objects.filter(subject_token=token)
                if not len(threads): continue
                thread = threads[0]
                new_url = '/archives/thread/%s/%s' % (thread.mailinglist.name, 
                                                      thread.subject_token)
                page_hit.url_path = new_url
                page_hit.save() 
